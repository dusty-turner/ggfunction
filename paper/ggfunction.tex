...
the total sample size; ties are handled correctly. The PP and QQ diagnostic geoms
...
This same probability-scale band is the basis for the PP and QQ diagnostic layers.

\subsection{\texorpdfstring{PP and QQ diagnostic plots: \texttt{geom\_ppplot()} and \texttt{geom\_qqplot()}}{PP and QQ diagnostic plots: geom\_ppplot() and geom\_qqplot()}}\label{pp-and-qq-diagnostic-plots-geom_ppplot-and-geom_qqplot}
...
quantiles \(x_{(i)}\) against the null quantiles \(Q_0(p_i)\). In both cases, agreement
with the null distribution corresponds to points falling close to the identity line.
Michael \citeyearpar{michael1983stabilized} discusses acceptance regions for PP, QQ, and
stabilized probability plots. The PP layer expects a CDF by default, while the QQ
layer expects a quantile function; alternate function sources are converted
internally using the same CDF/QF helpers used by the theoretical distribution geoms.
...
For QQ plots, the same probability limits are transformed back to the data scale:
...
Thus the two diagnostics show the same comparison on different scales: PP bands
are parallel probability bands, while QQ bands widen or narrow according to the
shape of \(Q_0\). The bands are finite-sample and distribution-free for a fully
...
\item
  \textbf{PP and QQ diagnostics} sort finite observations, compute plotting positions with \texttt{stats::ppoints(n,\ a\ =\ 1/2)}, and derive their DKW bands from the same half-width as \texttt{geom\_ecdf()}. The QQ ribbon transforms the probability-scale band through the supplied or derived quantile function.
\item
