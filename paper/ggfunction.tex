...
\texttt{geom\_pmf\_2d()} is the discrete counterpart: it evaluates a bivariate PMF over a lattice and renders the masses either as a balloon plot of points (\texttt{type\ =\ "point"}, the default, with size mapped to \texttt{after\_stat(prob)}; \texttt{ggplot2::scale\_size\_area()} is recommended so that mass is proportional to point area) or as a heatmap of tiles (\texttt{type\ =\ "tile"}, with fill mapped to \texttt{after\_stat(prob)}). The lattice is the product of per-axis supports--integers spanning \texttt{xlim} \(\times\) \texttt{ylim}, or exact values supplied via \texttt{support\_x} and \texttt{support\_y}--and the mass function follows the same \texttt{fun(c(x,\ y))} convention as \texttt{geom\_pdf\_2d()}. As in \texttt{geom\_pmf()}, the total lattice mass is validated, and \texttt{shade\_hdr} highlights the smallest set of lattice points whose total mass meets or exceeds the target coverage, rendering the remainder in grey; because a discrete distribution generally cannot achieve the target coverage exactly, the smallest HDR with coverage at least the target is used and the actual coverage is reported. Distributions with non-product support are accommodated by evaluating over a bounding lattice and returning zero off the support: zero-mass cells are dropped by default (\texttt{drop\_zeros\ =\ TRUE}), so a trinomial distribution, for example, renders only its simplex support.
