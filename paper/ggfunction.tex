...
\item
  An \textbf{empirical data family} providing layers for the empirical CDF, empirical quantile function, and empirical PMF, each computed directly from a sample and accompanied by simultaneous Kolmogorov--Smirnov confidence bands.
\end{enumerate}
...
Each geom accepts its native function type through the \texttt{fun} parameter (e.g., a PDF for \texttt{geom\_pdf()}, a CDF for \texttt{geom\_cdf()}), but most also accept alternate function types that are converted internally. For example, \texttt{geom\_cdf()} accepts a \texttt{pdf\_fun} argument and derives the CDF by numerical integration; \texttt{geom\_pdf()} accepts a \texttt{cdf\_fun} argument and derives the PDF by numerical differentiation; and \texttt{geom\_qf()} accepts both \texttt{cdf\_fun} and \texttt{pdf\_fun}, deriving the quantile function by root-finding. This cross-conversion feature lets users work with whichever characterization of a distribution is most natural--the PDF, CDF, survival function, or quantile function--regardless of which geom they wish to plot. Exactly one function source must be provided; supplying more than one is an error.
...
\texttt{geom\_pdf()} evaluates a user-supplied density and renders it as a filled area with an overlaid outline. Alternatively, a CDF can be supplied via \texttt{cdf\_fun}, from which the density is derived numerically by central finite differences. The underlying \texttt{StatPDF} validates the normalization property \eqref{eq:pdf} using \texttt{stats::integrate()}, issuing a diagnostic via \CRANpkg{cli} \citep{cli} if the integral departs from unity by more than a specified tolerance--a guard against accidentally passing an unnormalized function.
...
\textbf{Continuous survival (\texttt{geom\_survival()}).} In reliability theory and biostatistics, \(S(x) = 1 - F(x) = P(X > x)\) gives the probability that the event of interest has not yet occurred by time \(x\). \texttt{geom\_survival()} accepts a survival function directly via \texttt{fun}, or derives \(S\) from a CDF via \texttt{cdf\_fun} (computing \(1 - F(x)\)) or a PDF via \texttt{pdf\_fun} (integrating to obtain \(F\), then computing \(1 - F(x)\)).
...
\textbf{Continuous (\texttt{geom\_qf()}).} \texttt{geom\_qf()} evaluates a user-supplied quantile function (e.g., \texttt{qnorm}) over the unit interval \((0, 1)\). Alternatively, a CDF can be supplied via \texttt{cdf\_fun} (inverted numerically by root-finding) or a PDF via \texttt{pdf\_fun} (integrated to obtain the CDF, then inverted). Evaluation points are placed at Chebyshev nodes of the first kind, \(p_k = (1 - \cos((2k-1)\pi / 2n))/2\) for \(k = 1, \ldots, n\), which cluster near 0 and 1 where quantile functions are typically most curved and avoid evaluating at the exact endpoints, preventing \(\pm\infty\) for unbounded distributions.
...
\textbf{Discrete (\texttt{geom\_qf\_discrete()}).} The quantile function of a discrete distribution is a left-continuous step function on \([0, 1]\). \texttt{geom\_qf\_discrete()} accepts a quantile function directly via \texttt{fun} (e.g., \texttt{qbinom}), or derives it from a PMF via \texttt{pmf\_fun} (computing cumulative sums and inverting) or a CDF via \texttt{cdf\_fun} (evaluated on the integer support and inverted). It renders the result with horizontal segments on \([0, 1]\), dashed vertical jumps, closed circles at the bottom of each jump (the value is achieved), and open circles at the top (the next value is not yet reached). The \texttt{support} argument behaves as in \texttt{geom\_cdf\_discrete()}.
...
represents the instantaneous rate of failure at time \(x\), conditional on survival to that point \citep{casella2002statistical}. \texttt{geom\_hf()} accepts a hazard function directly via \texttt{fun}, or derives \(h\) from a PDF and CDF supplied via \texttt{pdf\_fun} and \texttt{cdf\_fun}. When only one of \texttt{pdf\_fun} or \texttt{cdf\_fun} is provided, the missing component is derived numerically (by integration or differentiation, respectively). The \texttt{args} parameter applies to all supplied functions; \texttt{pdf\_args} and \texttt{cdf\_args} provide overrides where the PDF and CDF require different parameterizations.
...
three geoms tabulate the empirical distribution via a shared helper that places mass
