...
\item
  A \textbf{probability distribution family} providing specialized layers for density, cumulative distribution, mass, quantile, discrete cumulative distribution, discrete quantile, discrete survival, survival, hazard, and cumulative hazard functions, as well as bivariate density and mass functions visualized through highest density regions and lattice displays.
\item
  An \textbf{empirical data family} providing layers for the empirical CDF, empirical quantile function, one-sample PP, QQ, and stabilized probability (SP) diagnostic plots, empirical PMF, and empirical cumulative hazard function, each computed directly from a sample and accompanied by simultaneous DKW/Massart confidence bands where appropriate. For right-censored survival data, the family additionally provides the Kaplan--Meier survival estimator \citep{kaplan1958nonparametric} with a simultaneous equal-precision Greenwood band \citep{greenwood1926natural, nair1984confidence} and the Nelson--Aalen cumulative hazard estimator \citep{nelson1972theory, aalen1978nonparametric} with pointwise normal bands.
\end{enumerate}
...
For raster layers, \texttt{raster\_aes\ =\ "fill"} keeps the default heatmap representation and continuous fill legend, while \texttt{raster\_aes\ =\ "alpha"} fixes the fill at dark gray and rescales finite function values to literal 0--1 alpha values.
...
Statistics education depends heavily on the visual language of probability distributions. Instructors shade tail areas to illustrate \(p\)-values, draw CDFs to connect probabilities with quantiles, and mark central regions to delineate confidence intervals. \pkg{ggfunction} provides a family of geoms--univariate and bivariate, continuous and discrete--that map directly onto the standard functions associated with a probability distribution, each with built-in shading support. Throughout, the user supplies an R function (e.g., \texttt{dnorm}, \texttt{pnorm}, \texttt{qnorm}) and a range; \pkg{ggfunction} handles evaluation, shading, and validation automatically.

Each geom accepts its native function type through the \texttt{fun} parameter (e.g., a PDF for \texttt{geom\_pdf()}, a CDF for \texttt{geom\_cdf()}), but most also accept alternate function types that are converted internally. In each case the missing characterization is derived numerically: PDFs are obtained by differentiation or hazard integration, CDFs by integration or interpolation, survival functions by the identity \(S = 1 - F\), and quantile functions by root-finding. Finite-support hazards can specify their integration origin with \texttt{hf\_lower}. This cross-conversion feature lets users work with whichever characterization of a distribution is most natural--the PDF, CDF, survival function, quantile function, or hazard function--regardless of which geom they wish to plot. Exactly one function source must be provided; supplying more than one is an error. The bivariate layers \texttt{geom\_pdf\_2d()} and \texttt{geom\_pmf\_2d()}, introduced below, accept only their native function type via \texttt{fun}. Table \ref{tab:cross-conversion} summarizes the available routes; the \texttt{cross-conversion} vignette documents the numerical methods and expected accuracy in detail.
...
\texttt{geom\_pdf()} evaluates a user-supplied density and renders it as a filled area with an overlaid outline. Alternatively, a CDF can be supplied via \texttt{cdf\_fun} (differentiated numerically), a survival function via \texttt{survival\_fun} (converted to a CDF via \(F = 1 - S\) and then differentiated), a quantile function via \texttt{qf\_fun} (inverted via interpolation to obtain a CDF, then differentiated), or a hazard function via \texttt{hf\_fun} (converted directly using the identity \(f(x) = h(x) \cdot e^{-H(x)}\)). For finite-support hazards, \texttt{hf\_lower} sets the lower integration limit for \(H\). The underlying \texttt{StatPDF} validates the normalization property \eqref{eq:pdf} using \texttt{stats::integrate()}, issuing a diagnostic via \CRANpkg{cli} \citep{cli} if the integral departs from unity by more than a specified tolerance--a guard against accidentally passing an unnormalized function.
...
\subsection{\texorpdfstring{Bivariate density functions: \texttt{geom\_pdf\_2d()}}{Bivariate density functions: geom\_pdf\_2d()}}\label{bivariate-density-functions-geom_pdf_2d}

The density family extends to bivariate distributions through \texttt{geom\_pdf\_2d()}, which visualizes a joint density \(f(x, y)\) through its highest density regions (HDRs)--the smallest regions of the plane containing specified probability masses--or as a raw density raster. Rather than reimplement the HDR computation, the layer is a thin, probability-facing wrapper around \CRANpkg{ggdensity} \citep{otto2023ggdensity}: with \texttt{type\ =\ "hdr"} (the default), filled regions are drawn by \texttt{ggdensity::geom\_hdr\_fun()}, and with \texttt{type\ =\ "hdr\_lines"} the region boundaries are drawn by \texttt{ggdensity::geom\_hdr\_lines\_fun()}, so HDR computation, probability labeling, and default aesthetics remain with \CRANpkg{ggdensity}. The density is supplied in \pkg{ggfunction}'s bivariate convention--\texttt{fun} receives a single vector \(v = (x, y)\) and returns one density value, with parameters passed via \texttt{args}--and is adapted internally to \CRANpkg{ggdensity}'s vectorized two-argument interface. The \texttt{probs} parameter sets the HDR coverage levels, defaulting to 99\%, 95\%, 80\%, and 50\%, and is ignored by \texttt{type\ =\ "raster"}, which evaluates the density on a grid using the alpha-raster mode of \texttt{geom\_function\_2d\_1d()}: dark-gray fill with density scaled to literal 0--1 alpha values. For iso-density contours not calibrated to probability content, \texttt{geom\_function\_2d\_1d()} remains the appropriate tool.
...
{\centering \includegraphics[width=0.85\linewidth]{ggfunction_files/figure-latex/pdf-2d-1} 

}

\caption{Bivariate normal densities drawn by \texttt{geom\_pdf\_2d()}: filled 50/80/95\% HDRs of the standard bivariate normal (left), HDR boundary lines for a correlated bivariate normal with covariance supplied via \texttt{args} (middle), and a raw density raster with density scaled to literal alpha (right).}\label{fig:pdf-2d}
\end{figure}
...
\texttt{geom\_pmf()} supports the same shading modes as \texttt{geom\_pdf()}. The \texttt{p} parameter shades lollipops up to the \(p\)-quantile; \texttt{p\_lower} and \texttt{p\_upper} define a two-sided interval; \texttt{shade\_outside\ =\ TRUE} inverts it; and \texttt{shade\_hdr} accepts one or more coverage levels, shading the smallest sets of mass points whose total probability meets or exceeds each target. Lollipops outside a \texttt{p}-based region are rendered at reduced opacity to preserve the distributional context, while \texttt{shade\_hdr} maps each support point's smallest containing HDR to \texttt{alpha} as an ordered factor (\texttt{after\_stat(probs)}), producing the same legend convention as the bivariate layers. Because a discrete distribution may not achieve the exact target coverages, \texttt{shade\_hdr} finds the smallest HDR with coverage \(\geq\) each requested level---a probability-threshold set \(\{x : p(x) \geq c\}\) that includes all mass points tied at the cutoff, so ties are never split and the achieved coverage almost always overshoots the target---and issues a diagnostic via \CRANpkg{cli} when an actual coverage differs from its target by more than 0.5 percentage points.
...
\caption{Shading modes for \texttt{geom\_pmf()}: the lower 80\% by cumulative probability, with unshaded lollipops rendered at reduced opacity (left), and the 50/80/95\% HDRs of a $\mathrm{Binomial}(10, 0.3)$ distribution, with each support point's smallest containing HDR mapped to alpha (right).}\label{fig:pmf-shading}
\end{figure}

\subsection{\texorpdfstring{Bivariate mass functions: \texttt{geom\_pmf\_2d()}}{Bivariate mass functions: geom\_pmf\_2d()}}\label{bivariate-mass-functions-geom_pmf_2d}

\texttt{geom\_pmf\_2d()} is the discrete counterpart: it evaluates a bivariate PMF over a lattice and renders the masses either as a balloon plot of points (\texttt{type\ =\ "point"}, the default, with size mapped to \texttt{after\_stat(prob)}; \texttt{ggplot2::scale\_size\_area()} is recommended so that mass is proportional to point area) or as a heatmap of tiles (\texttt{type\ =\ "tile"}, with fill mapped to \texttt{after\_stat(prob)}). The lattice is the product of per-axis supports--integers spanning \texttt{xlim} \(\times\) \texttt{ylim}, or exact values supplied via \texttt{support\_x} and \texttt{support\_y}--and the mass function follows the same \texttt{fun(c(x,\ y))} convention as \texttt{geom\_pdf\_2d()}. As in \texttt{geom\_pmf()}, the total lattice mass is validated. The \texttt{shade\_hdr} parameter accepts one or more coverage levels and highlights the corresponding highest density regions: each lattice point is assigned the smallest requested HDR that contains it, and the assignment is exposed as an ordered factor (\texttt{after\_stat(probs)}) mapped to \texttt{alpha} by default, reproducing the legend convention of \texttt{geom\_pdf\_2d()}; points outside all requested regions render nearly transparent. Because a discrete distribution generally cannot achieve a target coverage exactly, the smallest HDR with coverage at least each target is used---with lattice points of exactly equal mass always assigned together, never split---and the actual, typically overshooting, coverages are reported. Distributions with non-product support are accommodated by evaluating over a bounding lattice and returning zero off the support: zero-mass cells are dropped by default (\texttt{drop\_zeros\ =\ TRUE}), so a trinomial distribution, for example, renders only its simplex support.
...
\textbf{Continuous CDF (\texttt{geom\_cdf()}).} \texttt{geom\_cdf()} evaluates a user-supplied CDF (e.g., \texttt{pnorm}) and renders it as a line. Alternatively, a PDF can be supplied via \texttt{pdf\_fun} (integrated numerically), a survival function via \texttt{survival\_fun} (converted via \(F = 1 - S\)), a quantile function via \texttt{qf\_fun} (inverted via interpolation), or a hazard function via \texttt{hf\_fun} (the cumulative hazard is integrated from \texttt{hf\_lower} and then \(F = 1 - e^{-H}\)). It supports the same \texttt{p}, \texttt{p\_lower}/\texttt{p\_upper}, and \texttt{shade\_outside} shading parameters as \texttt{geom\_pdf()}.
...
\textbf{Discrete survival (\texttt{geom\_survival\_discrete()}).} For discrete distributions, \(S(x) = 1 - F(x)\) is itself a right-continuous step function. \texttt{geom\_survival\_discrete()} accepts a discrete survival function directly via \texttt{fun}, or derives \(S\) from a CDF via \texttt{cdf\_fun} or a PMF via \texttt{pmf\_fun} (computing cumulative sums and then \(1 - F\)). It renders the result with the same visual conventions as \texttt{geom\_cdf\_discrete()}. PMF-derived survival curves use \texttt{support} as the full computational support and \texttt{xlim} as a display filter.
...
\textbf{Discrete (\texttt{geom\_qf\_discrete()}).} The quantile function of a discrete distribution is a left-continuous step function on \([0, 1]\). \texttt{geom\_qf\_discrete()} accepts a quantile function directly via \texttt{fun} (e.g., \texttt{qbinom}), or derives it from a PMF via \texttt{pmf\_fun} (computing cumulative sums over the full support and inverting), a CDF via \texttt{cdf\_fun} (evaluated on the integer support and inverted), or a discrete survival function via \texttt{survival\_fun} (converted to a CDF via \(F = 1 - S\) and then inverted). It renders the result with horizontal segments on \([0, 1]\), dashed vertical jumps, closed circles at the bottom of each jump (the value is achieved), and open circles at the top (the next value is not yet reached). The \texttt{support} argument behaves as in \texttt{geom\_cdf\_discrete()}.
...
\texttt{geom\_chf()} renders \(H(x)\) as a curve. It accepts a cumulative hazard function directly via \texttt{fun}, or derives \(H\) from any other standard characterization: a hazard function via \texttt{hf\_fun} (integrated numerically from \texttt{hf\_lower}), a CDF via \texttt{cdf\_fun} (\(H = -\log(1 - F)\)), a survival function via \texttt{survival\_fun} (\(H = -\log S\)), a PDF via \texttt{pdf\_fun} (integrated to obtain \(F\), then transformed), or a quantile function via \texttt{qf\_fun} (interpolated to obtain \(F\), then transformed).
...
the same visual language as the probability distribution family to sample data. The
empirical CDF, quantile function, PMF, and cumulative hazard geoms tabulate the
empirical distribution via a shared helper that places mass \(c_k / n\) at each
distinct observed value \(x_k\), where \(c_k\) is the count of occurrences and \(n\) is
the total sample size; ties are handled correctly. The PP, QQ, and SP diagnostic geoms
use the related order-statistic representation of the same sample. Because the geoms
are standard \CRANpkg{ggplot2} layers, multiple groups can be overlaid by mapping the
\texttt{colour} aesthetic in the enclosing \texttt{ggplot()} call.
...
An optional simultaneous DKW/Massart confidence band is drawn around
...
0.95), and the ribbon's transparency by \texttt{conf\_alpha} (default 0.4).
...
\caption{Empirical CDFs for two groups from populations $\mathcal{N}(0,1)$ and $\mathcal{N}(2,1)$, with simultaneous 95\% DKW confidence bands. The bands are narrower for larger samples.}\label{fig:ecdf-basic}
\end{figure}
...
\eqref{eq:dkw-band}: since \(\hat{F}_n(x) - \varepsilon \leq F(x) \leq \hat{F}_n(x) +
...
\subsection{\texorpdfstring{PP, QQ, and SP diagnostic plots: \texttt{geom\_ppplot()}, \texttt{geom\_qqplot()}, and \texttt{geom\_spplot()}}{PP, QQ, and SP diagnostic plots: geom\_ppplot(), geom\_qqplot(), and geom\_spplot()}}\label{pp-qq-and-sp-diagnostic-plots-geom_ppplot-geom_qqplot-and-geom_spplot}

Suppose \(X_1,\ldots,X_n\) are to be compared with a fully specified null
distribution \(F_0\), with quantile function \(Q_0 = F_0^{\leftarrow}\). Let
\(x_{(1)} \leq \cdots \leq x_{(n)}\) denote the order statistics and define plotting
positions

\begin{equation}
p_i = \frac{i - 1/2}{n},\quad i = 1,\ldots,n.
\label{eq:ppoints}
\end{equation}

A probability-probability (PP) plot displays the null probabilities
\(F_0(x_{(i)})\) against \(p_i\). A quantile-quantile (QQ) plot displays the observed
quantiles \(x_{(i)}\) against the null quantiles \(Q_0(p_i)\). A stabilized probability
(SP) plot applies the arcsine-square-root variance-stabilizing transform

\begin{equation}
g(p) = \frac{2}{\pi} \sin^{-1} \sqrt{p}
\label{eq:sp-transform}
\end{equation}

to both PP coordinates, displaying \(g\{F_0(x_{(i)})\}\) against \(g(p_i)\); under the
null the transformed points have approximately equal variance across the entire unit
interval, so departures in the tails are as visually prominent as departures in the
middle. In each case, agreement with the null distribution corresponds to points
falling close to the identity line. Michael \citeyearpar{michael1983stabilized} discusses
acceptance regions for PP, QQ, and stabilized probability plots. The PP and SP
layers expect a CDF by default, while the QQ layer expects a quantile function;
alternate function sources are converted internally using the same CDF/QF helpers
used by the theoretical distribution geoms.

Under \(H_0: F = F_0\), the probability integral transform gives
\(F_0(X_i) \stackrel{\mathrm{iid}}{\sim} \mathrm{Unif}(0,1)\). Therefore
\eqref{eq:dkw-band} gives a simultaneous DKW band on the probability scale. For PP
plots, the ribbon is simply

\begin{equation}
\max(0, p_i - \varepsilon) \leq F_0(x_{(i)}) \leq \min(1, p_i + \varepsilon).
\label{eq:pp-band}
\end{equation}

For SP plots, the monotone transform \(g\) of \eqref{eq:sp-transform} is applied to
both sides of \eqref{eq:pp-band}, carrying the same simultaneous band to the
stabilized scale. For QQ plots, the probability limits are transformed back to the
data scale:

\begin{equation}
Q_0\{\max(0, p_i - \varepsilon)\} \leq x_{(i)}
\leq Q_0\{\min(1, p_i + \varepsilon)\}.
\label{eq:qq-band}
\end{equation}

Thus the three diagnostics show the same comparison on different scales: PP bands
are parallel probability bands, QQ bands widen or narrow according to the shape of
\(Q_0\), and SP bands carry the parallel probability bands to the variance-stabilized
scale. The bands are finite-sample and distribution-free for a fully
specified null. When parameters are estimated from the data, the plots remain useful
visual diagnostics, but the DKW bands are conservative rather than Lilliefors-adjusted.
...
transformation \(H = -\log(1 - F)\) to the DKW bounds \eqref{eq:dkw-band}. Because
...
\subsection{\texorpdfstring{Kaplan--Meier survival curves: \texttt{geom\_ecdf\_km()}}{Kaplan--Meier survival curves: geom\_ecdf\_km()}}\label{kaplanmeier-survival-curves-geom_ecdf_km}

The empirical geoms above assume complete (uncensored) data. In practice,
survival data are often right-censored: some subjects leave the study before
the event is observed. The Kaplan--Meier product-limit estimator
\citep{kaplan1958nonparametric} handles this by adjusting the risk set at each
event time:
...
where \(d_j\) is the number of events at time \(t_j\) and \(n_j\) is the number at
risk just before \(t_j\). \texttt{geom\_ecdf\_km()} renders this as a decreasing step
function, with a simultaneous equal-precision Greenwood confidence band \citep{greenwood1926natural, nair1984confidence} and censoring marks at
observations where the event was not observed. The data require two aesthetics:
\texttt{x} (the observed time) and \texttt{status} (1 = event, 0 = censored).
...
{\centering \includegraphics[width=0.85\linewidth]{ggfunction_files/figure-latex/km-censored-1} 

}

\caption{Kaplan--Meier survival curves for simulated censored data. Event times follow $\text{Exp}(0.5)$ and censoring times follow $\text{Exp}(0.2)$. The left panel shows the KM estimate with a Greenwood 95\% simultaneous equal-precision band and censoring marks; the right panel overlays the true survival curve (red).}\label{fig:km-censored}
\end{figure}

The theoretical survival curve (red) lies within the Greenwood EP confidence band,
confirming consistency with the \(\text{Exp}(0.5)\) model.

\subsection{\texorpdfstring{Nelson--Aalen cumulative hazard: \texttt{geom\_echf\_na()}}{Nelson--Aalen cumulative hazard: geom\_echf\_na()}}\label{nelsonaalen-cumulative-hazard-geom_echf_na}

The Nelson--Aalen estimator \citep{nelson1972theory, aalen1978nonparametric} is the
censored-data analogue of the empirical cumulative hazard:
...
with variance estimated by \(\widehat{\mathrm{Var}}[\hat{H}(t)] = \sum_{t_j
\le t} d_j / n_j^2\). \texttt{geom\_echf\_na()} renders this as an increasing step
function with a pointwise normal confidence band.
...
{\centering \includegraphics[width=0.85\linewidth]{ggfunction_files/figure-latex/na-censored-1} 

}

\caption{Nelson--Aalen cumulative hazard estimates. The same censored data as in the KM figure. The left panel shows the Nelson--Aalen estimate with a 95\% pointwise normal confidence band; the right panel overlays the theoretical $H(x) = 0.5x$ (red).}\label{fig:na-censored}
\end{figure}

The theoretical cumulative hazard (red) threads through the confidence band,
consistent with the generating model.

\subsection{Validity of the confidence band}\label{validity-of-the-confidence-band}

The DKW inequality \eqref{eq:dkw-band} is a \emph{finite-sample} result: it
...
and nominal level \(1 - \alpha\), we draw \(B = 10{,}000\) independent samples from
...
\item
  \textbf{PP, QQ, and SP diagnostics} sort finite observations, compute plotting positions with \texttt{stats::ppoints(n,\ a\ =\ 1/2)}, and derive their DKW bands from the same half-width as \texttt{geom\_ecdf()}. The QQ ribbon transforms the probability-scale band through the supplied or derived quantile function, and the SP layer applies the arcsine-square-root transform \eqref{eq:sp-transform} to both the plotted coordinates and the band limits.
\item
...
\item
  \CRANpkg{ggdensity} \citep{otto2023ggdensity}. This package improves bivariate density visualization in \CRANpkg{ggplot2}, with a particular focus on highest density regions (HDRs). \pkg{ggfunction}'s \texttt{shade\_hdr} parameter in \texttt{geom\_pdf()} follows the HDR computation approach introduced there: normalizing density values, sorting in descending order, and accumulating until the target coverage is reached. More directly, \texttt{geom\_pdf\_2d()} delegates HDR computation and rendering to \CRANpkg{ggdensity}'s \texttt{geom\_hdr\_fun()} and \texttt{geom\_hdr\_lines\_fun()}, adapting the \texttt{fun(c(x,\ y))} convention shared by \pkg{ggfunction}'s bivariate layers to \CRANpkg{ggdensity}'s vectorized two-argument interface; its raster mode evaluates the same bivariate density convention directly on a grid.
\item
