...
\item
  An \textbf{empirical data family} providing layers for the empirical CDF, empirical quantile function, empirical PMF, and empirical cumulative hazard function, each computed directly from a sample and accompanied by simultaneous Kolmogorov--Smirnov confidence bands. For right-censored survival data, the family additionally provides the Kaplan--Meier survival estimator \citep{kaplan1958nonparametric} and Nelson--Aalen cumulative hazard estimator \citep{nelson1972theory, aalen1978nonparametric} with simultaneous equal-precision confidence bands \citep{nair1984confidence}.
\end{enumerate}
...
function, with a simultaneous Greenwood confidence band \citep{greenwood1926natural, nair1984confidence} and censoring marks at
...
\caption{Kaplan--Meier survival curves for simulated censored data. Event times follow $\text{Exp}(0.5)$ and censoring times follow $\text{Exp}(0.2)$. The left panel shows the KM estimate with Greenwood 95\% simultaneous confidence band and censoring marks; the right panel overlays the true survival curve (red).}\label{fig:km-censored}
\end{figure}

The theoretical survival curve (red) lies within the Greenwood confidence band,
...
function with a simultaneous confidence band \citep{nair1984confidence}.
...
\caption{Nelson--Aalen cumulative hazard estimates. The same censored data as in the KM figure. The left panel shows the Nelson--Aalen estimate with 95\% simultaneous confidence band; the right panel overlays the theoretical $H(x) = 0.5x$ (red).}\label{fig:na-censored}
\end{figure}
