...
The density family extends to bivariate distributions through \texttt{geom\_pdf\_2d()}, which visualizes a joint density \(f(x, y)\) through its highest density regions (HDRs)--the smallest regions of the plane containing specified probability masses. Rather than reimplement this computation, the layer is a thin, probability-facing wrapper around \CRANpkg{ggdensity} \citep{otto2023ggdensity}: with \texttt{type\ =\ "hdr"} (the default), filled regions are drawn by \texttt{ggdensity::geom\_hdr\_fun()}, and with \texttt{type\ =\ "hdr\_lines"} the region boundaries are drawn by \texttt{ggdensity::geom\_hdr\_lines\_fun()}, so HDR computation, probability labeling, and default aesthetics remain with \CRANpkg{ggdensity}. The density is supplied in \pkg{ggfunction}'s bivariate convention--\texttt{fun} receives a single vector \(v = (x, y)\) and returns one density value, with parameters passed via \texttt{args}--and is adapted internally to \CRANpkg{ggdensity}'s vectorized two-argument interface. The \texttt{probs} parameter sets the HDR coverage levels, defaulting to 99\%, 95\%, 80\%, and 50\%. For raw density heatmaps or iso-density contours not calibrated to probability content, \texttt{geom\_function\_2d\_1d()} remains the appropriate tool.
...
\caption{Highest density regions of bivariate normal densities drawn by \texttt{geom\_pdf\_2d()}, which delegates to ggdensity: filled 50/80/95\% HDRs of the standard bivariate normal (left) and HDR boundary lines for a correlated bivariate normal with covariance supplied via \texttt{args} (right).}\label{fig:pdf-2d}
\end{figure}
...
\texttt{geom\_pmf\_2d()} is the discrete counterpart: it evaluates a bivariate PMF over a lattice and renders the masses either as a balloon plot of points (\texttt{type\ =\ "point"}, the default, with size mapped to \texttt{after\_stat(prob)}; \texttt{ggplot2::scale\_size\_area()} is recommended so that mass is proportional to point area) or as a heatmap of tiles (\texttt{type\ =\ "tile"}, with fill mapped to \texttt{after\_stat(prob)}). The lattice is the product of per-axis supports--integers spanning \texttt{xlim} \(\times\) \texttt{ylim}, or exact values supplied via \texttt{support\_x} and \texttt{support\_y}--and the mass function follows the same \texttt{fun(c(x,\ y))} convention as \texttt{geom\_pdf\_2d()}. As in \texttt{geom\_pmf()}, the total lattice mass is validated. The \texttt{shade\_hdr} parameter accepts one or more coverage levels and highlights the corresponding highest density regions: each lattice point is assigned the smallest requested HDR that contains it, and the assignment is exposed as an ordered factor (\texttt{after\_stat(probs)}) mapped to \texttt{alpha} by default, reproducing the legend convention of \texttt{geom\_pdf\_2d()}; points outside all requested regions render nearly transparent. Because a discrete distribution generally cannot achieve a target coverage exactly, the smallest HDR with coverage at least each target is used and the actual coverages are reported. Distributions with non-product support are accommodated by evaluating over a bounding lattice and returning zero off the support: zero-mass cells are dropped by default (\texttt{drop\_zeros\ =\ TRUE}), so a trinomial distribution, for example, renders only its simplex support.
...
\item
  \CRANpkg{ggdensity} \citep{otto2023ggdensity}. This package improves bivariate density visualization in \CRANpkg{ggplot2}, with a particular focus on highest density regions (HDRs). \pkg{ggfunction}'s \texttt{shade\_hdr} parameter in \texttt{geom\_pdf()} follows the HDR computation approach introduced there: normalizing density values, sorting in descending order, and accumulating until the target coverage is reached. More directly, \texttt{geom\_pdf\_2d()} is a thin wrapper that delegates HDR computation and rendering to \CRANpkg{ggdensity}'s \texttt{geom\_hdr\_fun()} and \texttt{geom\_hdr\_lines\_fun()}, adapting the \texttt{fun(c(x,\ y))} convention shared by \pkg{ggfunction}'s bivariate layers to \CRANpkg{ggdensity}'s vectorized two-argument interface.
\item
