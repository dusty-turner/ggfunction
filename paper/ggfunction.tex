...
\textbf{Continuous CDF (\texttt{geom\_cdf()}).} \texttt{geom\_cdf()} evaluates a user-supplied CDF (e.g., \texttt{pnorm}) and renders it as a line. It supports the same \texttt{p}, \texttt{p\_lower}/\texttt{p\_upper}, and \texttt{shade\_outside} shading parameters as \texttt{geom\_pdf()}.
...
\textbf{Continuous survival (\texttt{geom\_survival()}).} In reliability theory and biostatistics, \(S(x) = 1 - F(x) = P(X > x)\) gives the probability that the event of interest has not yet occurred by time \(x\). \texttt{geom\_survival()} accepts a CDF and plots its complement.
...
\textbf{Discrete survival (\texttt{geom\_survival\_discrete()}).} For discrete distributions, \(S(x) = 1 - F(x)\) is itself a right-continuous step function. \texttt{geom\_survival\_discrete()} computes \(S\) from a PMF and renders it with the same visual conventions as \texttt{geom\_cdf\_discrete()}. The \texttt{support} argument behaves as in \texttt{geom\_cdf\_discrete()}.
...
\textbf{Continuous (\texttt{geom\_qf()}).} \texttt{geom\_qf()} evaluates a user-supplied quantile function (e.g., \texttt{qnorm}) over the unit interval \((0, 1)\). Evaluation points are placed at Chebyshev nodes of the first kind, \(p_k = (1 - \cos((2k-1)\pi / 2n))/2\) for \(k = 1, \ldots, n\), which cluster near 0 and 1 where quantile functions are typically most curved and avoid evaluating at the exact endpoints, preventing \(\pm\infty\) for unbounded distributions.
...
\textbf{Discrete (\texttt{geom\_qf\_discrete()}).} The quantile function of a discrete distribution is a left-continuous step function on \([0, 1]\). \texttt{geom\_qf\_discrete()} takes a PMF, computes the cumulative sum, and renders the inverse function with horizontal segments on \([0, 1]\), dashed vertical jumps, closed circles at the bottom of each jump (the value is achieved), and open circles at the top (the next value is not yet reached). The \texttt{support} argument behaves as in \texttt{geom\_cdf\_discrete()}.
...
the step function by default. The DKW inequality \citep{massart1990tight} implies that
...
The DKW inequality \eqref{eq:ks-band} guarantees that the simultaneous
confidence band contains the true CDF everywhere with probability at least
\(1 - \alpha\). Because the KS statistic \(D_n = \sup_x |\hat{F}_n(x) - F(x)|\)
has a distribution-free null distribution for continuous \(F\), the coverage
guarantee holds regardless of the shape of the true distribution.
...
\caption{Empirical simultaneous coverage of the KS confidence band over 2,000 simulations from $\mathcal{N}(0,1)$. Dashed lines show the nominal levels; solid curves show empirical coverage. Coverage is everywhere at or above nominal and converges toward nominal as $n$ grows, consistent with the asymptotic tightness of the DKW bound \citep{massart1990tight}.}\label{fig:sim-coverage}
\end{figure}

Empirical coverage (solid lines) lies above each dashed nominal threshold
across all tested sample sizes. The slight conservatism at small \(n\) reflects
the fact that the DKW bound is an inequality; convergence toward the nominal
level as \(n\) grows is consistent with the asymptotic tightness established by
\citet{massart1990tight}.
