...
For discrete distributions, the survival function \(S(x) = 1 - F(x) = P(X > x)\) is itself a right-continuous step function. \texttt{geom\_survival\_discrete()} computes \(S\) from a PMF and renders it with the same visual conventions as \texttt{geom\_cdf\_discrete()}: horizontal segments, dashed vertical jumps, open circles at the pre-drop value, and closed circles at the achieved (lower) value. The \texttt{support} argument is accepted and behaves as in \texttt{geom\_cdf\_discrete()}.
