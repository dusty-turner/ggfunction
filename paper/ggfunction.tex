...
\caption{Bivariate normal densities drawn by \texttt{geom\_pdf\_2d()}: filled 50/80/95\% HDRs of the standard bivariate normal (left), HDR boundary lines for a correlated bivariate normal with covariance supplied via \texttt{args} (middle), and a raw density raster with alpha mapped to density (right).}\label{fig:pdf-2d}
\end{figure}
