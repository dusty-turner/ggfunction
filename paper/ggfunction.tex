...
\item
  \textbf{Vector fields} (\(\mathbb{R}^2 \to \mathbb{R}^2\)): streamlines computed by numerical integration of the field.
\end{itemize}
...
The \texttt{normalize} parameter (default \texttt{TRUE}) scales streamline lengths relative to the grid spacing, producing a uniform visual density. Setting \texttt{normalize\ =\ FALSE} allows streamlines to reflect the true magnitude of the field through their arc length, controlled by the integration time \texttt{T}. Users whose primary interest is vector fields will find that \pkg{ggvfields} provides a richer collection of tools, including gradient fields, stream plots, and potential functions.
...
The quantile function \(Q(p) = F^{-1}(p) = \inf\{x : F(x) \geq p\}\) inverts the CDF. \texttt{geom\_qf()} evaluates a user-supplied quantile function (e.g., \texttt{qnorm}) over the unit interval \((0, 1)\).
...
\subsection{\texorpdfstring{Discrete CDFs: \texttt{geom\_discrete\_cdf()}}{Discrete CDFs: geom\_discrete\_cdf()}}\label{discrete-cdfs-geom_discrete_cdf}

For discrete distributions, the CDF is a step function. \texttt{geom\_discrete\_cdf()} takes a PMF as input, computes cumulative sums, and renders the result as step-function vertices constructed by the internal \texttt{build\_step\_polygon()} helper. Like \texttt{geom\_cdf()}, it supports \texttt{p}-based shading.
...
\caption{The discrete CDF of a $\mathrm{Binomial}(10, 0.5)$ distribution with the region below $p = 0.9$ shaded.}\label{fig:discrete-cdf}
\end{figure}
