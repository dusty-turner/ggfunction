...
|  | `geom_pmf_2d()` |  | Bivariate PMF (balloon plot or tile heatmap) |
...
<img src="man/figures/readme-2d-1d-alpha-raster-1.png" alt="" width="60%" />
...
`geom_pdf_2d()` visualizes a theoretical bivariate density through its
highest density regions or as a raw density raster. It is a thin wrapper
around [**ggdensity**](https://jamesotto852.github.io/ggdensity/) for
HDRs: the function is supplied in **ggfunction**’s `fun(c(x, y))`
convention and adapted internally, while HDR computation, probability
labels, and default aesthetics are delegated to
`ggdensity::geom_hdr_fun()` (filled regions, the default `type = "hdr"`)
or `ggdensity::geom_hdr_lines_fun()` (`type = "hdr_lines"`). With
`type = "raster"`, the density is evaluated on the requested grid and
drawn with the alpha-raster mode of `geom_function_2d_1d()`: a dark-gray
fill with alpha scaled to literal 0–1 transparency. Parameters pass via
`args` as usual.
...
<img src="man/figures/readme-pdf-2d-1.png" alt="" width="60%" />
...
<img src="man/figures/readme-pdf-2d-lines-1.png" alt="" width="60%" />
...
<img src="man/figures/readme-pdf-2d-raster-1.png" alt="" width="60%" />

For arbitrary iso-density contours not calibrated to probability
...
$p$-quantile (the unshaded region is rendered at reduced opacity). Here
we shade the lower 80% of a $\text{Binomial}(10, 0.5)$ distribution.
...
legend—the same convention as `geom_pmf_2d()`. For a symmetric unimodal
...
ties: each HDR is the probability-threshold set $\{x : p(x) \ge c\}$, so
support points with exactly equal mass are always included or excluded
together—another source of overshoot.
...
passed via `args`. The default `type = "point"` renders a balloon plot
with size encoding the probability mass; `scale_size_area()` makes area
proportional to mass and zero mass vanish.
