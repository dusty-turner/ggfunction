...
|-----------------|--------------------------------------------------------------------|--------------------------------------------------------------|----------------------------------------------------|
| **Dimensional** | [`geom_function_1d_1d()`](/reference/geom_function_1d_1d.md)       | $\left. {\mathbb{R}}\rightarrow{\mathbb{R}} \right.$         | Scalar functions with optional interval shading    |
|                 | [`geom_function_1d_2d()`](/reference/geom_function_1d_2d.md)       | $\left. {\mathbb{R}}\rightarrow{\mathbb{R}}^{2} \right.$     | Parametric curves                                  |
|                 | [`geom_function_2d_1d()`](/reference/geom_function_2d_1d.md)       | $\left. {\mathbb{R}}^{2}\rightarrow{\mathbb{R}} \right.$     | Scalar fields (raster, contour, filled contour)    |
|                 | [`geom_function_2d_2d()`](/reference/geom_function_2d_2d.md)       | $\left. {\mathbb{R}}^{2}\rightarrow{\mathbb{R}}^{2} \right.$ | Vector field streamlines                           |
...
| **Data**        | [`geom_ecdf()`](/reference/geom_ecdf.md)                           |                                                              | Empirical CDF with KS confidence ribbon            |
...
function $\left. f:{\mathbb{R}}\rightarrow{\mathbb{R}} \right.$ at `n`
equally-spaced points over `xlim` and draws it as a curve, extending
...
parametric curve ${\mathbf{γ}}(t) = \left( x(t),\, y(t) \right)$ over
the range `tlim` with step size `dt`, coloring the path by the time
...
${\mathbf{γ}}(t) = (\cos t\,/\,\left( 1 + \sin^{2}t \right),\;\sin t\cos t\,/\,\left( 1 + \sin^{2}t \right))$.
...
scalar field $\left. f:{\mathbb{R}}^{2}\rightarrow{\mathbb{R}} \right.$
on an $n \times n$ grid over `xlim` $\times$`ylim`. The function must
...
example below shows a Gaussian bump
$f(x,y) = \exp\!( - \left( x^{2} + y^{2} \right)/2)$ rendered as a
...
a vector field
$\left. \mathbf{F}:{\mathbb{R}}^{2}\rightarrow{\mathbb{R}}^{2} \right.$
via [**ggvfields**](https://github.com/dusty-turner/ggvfields). The
...
![](reference/figures/readme-geom-pmf-hdr-2-1.png)
...
$\text{Bernoulli}(0.3)$ draws. The support is $\{ 0,0.1,0.2,\ldots,1\}$
and the PMF is binomial:
$P\left( \bar{X} = k/10 \right) = \left( \frac{10}{k} \right)0.3^{k}0.7^{10 - k}$.
...
The cumulative distribution function $F(x) = P(X \leq x)$ is available
...
![](reference/figures/readme-discrete-cdf-pois-1.png)
...
Accepts a CDF via `fun`. The following example shows the survival
function of an $\text{Exponential}(0.5)$ distribution, which decays as
$S(x) = e^{-0.5x}$.
...
[`geom_cdf_discrete()`](/reference/geom_cdf_discrete.md). The following
example plots the $\text{Binomial}(10,0.5)$ survival function.
...
The quantile function $Q(p) = \inf\{ x:F(x) \geq p\}$ inverts the CDF.
...
is not yet reached). The following example plots the
...
conditional on survival to that point. It requires both a PDF
(`pdf_fun`) and a CDF (`cdf_fun`). Shared parameters go in `args`; use
...
simultaneous Kolmogorov-Smirnov confidence ribbon is drawn by default;
set `conf_int = FALSE` to suppress it.
...
function $Q_{n}(p) = \inf\{ x:{\widehat{F}}_{n}(x) \geq p\}$ as a
left-continuous step function on $\lbrack 0,1\rbrack$, using the same
visual conventions as
[`geom_qf_discrete()`](/reference/geom_qf_discrete.md). The confidence
ribbon inverts the KS ECDF band: at probability $p$ the band spans
$\left\lbrack Q_{n}(p - \varepsilon),Q_{n}(p + \varepsilon) \right\rbrack$.
...
true quantile function $Q(p)$ with probability $\geq 1 - \alpha$
regardless of the null, overlaying a parametric $Q_{0}(p)$ turns the
plot into a visual goodness-of-fit test: if $Q_{0}$ lies entirely within
the band the data are consistent with the hypothesized model; if it
exits the band there is evidence against it. The comparison below fits a
...
overlays the fitted $Q_{0}$ as a smooth line.
...
![](reference/figures/readme-eqf-gof-1.png)
...
probability mass function—placing mass $c_{k}/n$ at each distinct
observed value $x_{k}$—and renders it as a lollipop chart using the same
