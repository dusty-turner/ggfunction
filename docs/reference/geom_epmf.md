...
lollipop chart, using the same visual conventions as
[`geom_pmf()`](/reference/geom_pmf.md): vertical segments from zero to
each probability value, capped with points.
...
- stick_linewidth:
  ...
  Linewidth of the vertical segments. Defaults to `0.25`.

- stick_linetype:

  Linetype of the vertical segments. Defaults to `"dashed"`.

## Value

A list of two ggplot2 layers: a blank layer that anchors the y-axis at 0
and the main lollipop layer.
