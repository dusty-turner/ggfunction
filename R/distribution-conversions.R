...
hf_to_chf <- function(hf_fun, lower = -Inf, support = NULL) {
  support <- validate_support_1d(support %||% c(lower, Inf))
  lower <- resolve_hf_lower(lower, support)
  warned <- FALSE
  ...
  out <- function(x) {
    vapply(x, function(xi) {
      ...
      if (is.finite(support[2]) && xi >= support[2]) xi <- support[2]
...
hf_to_pdf <- function(hf_fun, lower = -Inf, support = NULL) {
  ...
  out <- function(x) {
    vapply(x, function(xi) {
      if (is.na(xi)) return(NA_real_)
      if (is.finite(lower) && xi < lower) return(0)
      if (is.finite(support[2]) && xi > support[2]) return(0)
      H_x <- chf_fun(xi)
...
as_hf_1d <- function(fun = NULL, pdf_fun = NULL, cdf_fun = NULL,
  ...
  if (!is.null(pdf_fun) && !is.null(cdf_fun)) {
    ...
    f <- distribution_fun(pdf_fun, utils::modifyList(args, pdf_args), "pdf_fun")
    F <- pdf_to_cdf(f, support = support)
    ...
    out <- function(x) {
      s <- S(x)
      ifelse(s > 0, f(x) / s, NaN)
    }
    return(tag_distribution_route(out, "survival->hf", TRUE))
  } else {
  ...
  tag_distribution_route(out, "pdf,cdf->hf",
                         isTRUE(attr(f, "ggfunction_approximate")) ||
                           isTRUE(attr(F, "ggfunction_approximate")))
}
