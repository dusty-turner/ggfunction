...
$H(x) = \int_{\ell}^{x} h(t)\, dt$, where $\ell$ is the support origin. By
default `ggfunction` uses $\ell = -\infty$; for finite-support hazards, pass
`hf_lower` to the plotting geom (for example, `hf_lower = 0` for exponential or
Weibull hazards). Values below a finite `hf_lower` have CDF, PDF, and cumulative
hazard equal to 0. The CDF follows as $F(x) = 1 - e^{-H(x)}$. Internally,
`hf_to_cdf()` computes $H(x)$ via `stats::integrate()` and applies the
...
p2 <- ggplot() +
  ...
  geom_pdf(fun = dnorm, xlim = c(-3, 3), color = "red", fill = NA,
...
p1 <- ggplot() +
  geom_pdf(hf_fun = h_weibull, support = c(0, Inf), xlim = c(0.01, 5), args = list(shape = 2, scale = 1)) +
  ggtitle("PDF (from hazard)")

p2 <- ggplot() +
  geom_cdf(hf_fun = h_weibull, support = c(0, Inf), xlim = c(0.01, 5), args = list(shape = 2, scale = 1)) +
  ggtitle("CDF (from hazard)")

p3 <- ggplot() +
  geom_chf(hf_fun = h_weibull, support = c(0, Inf), xlim = c(0.01, 5), args = list(shape = 2, scale = 1)) +
...
- **Hazard to CDF** (`hf_to_cdf`): $O(n)$ calls to `integrate()`, comparable
  to `pdf_to_cdf`. Use `hf_lower` for finite-support hazards so integration
...
- **PMF to CDF**: cumulative summation of the PMF values over the full integer
  or explicit support. `xlim` can then filter the displayed range. This is exact
