...
```{r lemniscate, fig.height = 4, warning = FALSE}
...
The same comparison can be made directly with PP and QQ diagnostic plots:

```{r pp-qq-diagnostics, fig.height = 4}
pp <- ggplot(df, aes(sample = x)) +
  geom_ppplot(fun = pnorm, null_type = "continuous") +
  coord_equal() +
  labs(title = "PP plot")

qq <- ggplot(df, aes(sample = x)) +
...
```{r qq-diagnostics-styling, fig.height = 4}
qq_black <- ggplot(df, aes(sample = x)) +
  geom_qqplot(fun = qnorm, colour = "black") +
  coord_equal() +
  labs(title = "Black points")

qq_spectral <- ggplot(df, aes(sample = x)) +
  geom_qqplot(fun = qnorm) +
