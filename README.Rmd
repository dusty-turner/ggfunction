...
`geom_pmf_2d()` evaluates a bivariate mass function on a discrete lattice---integers spanning `xlim` $\times$ `ylim` by default, or exact values via `support_x`/`support_y`. The function uses the same `fun(c(x, y))` convention as the other bivariate layers, with parameters passed via `args`. The default `type = "tile"` renders a heatmap with fill encoding the probability mass.
...
**Balloon plot.** `type = "point"` encodes mass by point size instead; `scale_size_area()` makes area proportional to mass.
...
**Highest density region.** As with `geom_pmf()`, `shade_hdr` highlights the smallest set of lattice points containing the target probability mass, greying out the rest. The exact coverage may not be achievable for a discrete distribution, in which case the smallest HDR with at least the target coverage is used and a message reports the actual coverage.
