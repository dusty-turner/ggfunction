...
**PDF from a hazard function.** Given a Weibull hazard function $h(x)$, `geom_pdf()` recovers the density via the identity $f(x) = h(x)\exp(-H(x))$.
...
`geom_ecdf_km()` computes the Kaplan-Meier product-limit estimator from right-censored data and renders it as a decreasing step function. The `status` aesthetic indicates whether each observation is an event (1) or censored (0). A simultaneous Greenwood confidence band and censoring marks are drawn by default.
...
**Goodness-of-fit overlay.** Overlaying the theoretical survival curve on the Kaplan-Meier estimate provides a visual check: if the curve lies within the Greenwood confidence band, the data are consistent with the model. Here we overlay the true $\text{Exp}(0.5)$ survival function.
...
`geom_echf_na()` computes the Nelson-Aalen cumulative hazard estimator from right-censored data and renders it as an increasing step function with a simultaneous confidence band.
