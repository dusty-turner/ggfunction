...
`geom_function_2d_2d()` visualizes a vector field $\mathbf{F}\colon \mathbb{R}^2 \to \mathbb{R}^2$ as streamlines---integral curves that are everywhere tangent to the field---computed by numerical integration via [**ggvfields**](https://github.com/dusty-turner/ggvfields). The function should accept a length-2 vector and return a length-2 vector. The following example shows the rotation field $\mathbf{F}(x,y) = (-y,\, x)$, whose streamlines are circles.
