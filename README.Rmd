...
`geom_discrete_cdf()` takes a PMF as input, accumulates it into a step-function CDF, and renders it as a staircase. Like `geom_cdf()`, it supports `p`-based shading via the `p` parameter. Here we plot the $\text{Binomial}(10, 0.5)$ CDF and shade the region below $p = 0.9$.
