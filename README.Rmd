...
`geom_ppplot()` and `geom_qqplot()` compare sample order statistics to a fully specified null distribution. Both draw 95% DKW/Massart confidence bands by default, add a dashed identity line, and map point colour to the sample value (PP) or plotting position (QQ). They use ggplot2's default continuous colour scale unless you add your own.
...
**Black points.** Set a fixed colour when you want an uncolored diagnostic plot.
