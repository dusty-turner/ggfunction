...
**Viridis HDR colour.** `shade_hdr` maps HDR membership to `alpha` by default. When the alpha encoding is too subtle, map `after_stat(probs)` to a discrete color scale instead and fix `alpha = 1`; the sticks and points share the HDR colors.
...
**Viridis HDR fill.** If the alpha encoding is too subtle, map the HDR levels to `fill` instead with a fillable point shape and a discrete viridis palette, fixing `alpha = 1` to turn off the default encoding. Probability mass stays encoded by point size.
